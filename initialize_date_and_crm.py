...
def initialize_date_and_crm_info():
    ...
    previous_cutoff_end = (last_period_start_date-pd.DateOffset(months=reference_period_ignored_months)-pd.DateOffset(days=1)).date()
    last_period_months = (last_cutoff_end.year - last_cutoff_start.year) * 12 + (last_cutoff_end.month - last_cutoff_start.month)
    
    last_cutoff_start_str = convert_date_to_str(last_cutoff_start)
    last_cutoff_end_str = convert_date_to_str(last_cutoff_end)
    
    st.write(f'La période de référence pour le calcul du nouveau CRM correspond à la période du {last_cutoff_start_str} au {last_cutoff_end_str}.')
    if mode == "Nouvelle souscription" and last_period_months < 10:
        st.write(f"Le CRM ne peut pas s'améliorer car elle s'étale sur moins de 10 mois.")
