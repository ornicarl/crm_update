...
def fill_antecedents(
        ...
        if calculation_method == "Détaillée":
            ...
            if previousRespClaimCount == 0:
                if totalRespClaimCount_default > 0:
                    ...
                    if totalRespClaimCount_default == 0:
                        st.write(
                            f"Aucun autre sinistre responsable survenu du {last_cutoff_start_str} au {last_cutoff_end_str}. Pas de dégradation du CRM mais perte de l'ancienneté bonus 50."
                        )
                    else:
                        if totalRespClaimCount_default == 1:
                            st.write(
                                f"{totalRespClaimCount_default} autre sinistre responsable survenu du {last_cutoff_start_str} au {last_cutoff_end_str}. Dégradation du CRM."
                            )
                        else:
                            st.write(
                                f"{totalRespClaimCount_default} autres sinistres responsables survenus du {last_cutoff_start_str} au {last_cutoff_end_str}. Dégradation du CRM."
                            )
                    # Fully Responsible Claims
                    fullyRespClaimCount_default = (
                        (df_claims["claim_resp"] == "Totale")
    ...
    with fullyRespClaim_col:
        # Fully Responsible Claims
        st.write(
            f"Sinistres du {last_cutoff_start_str} au {last_cutoff_end_str} - Responsabilité totale"
        )
    ...
    with partiallyRespClaim_col:
        # Partially Responsible Claims
        st.write(
            f"Sinistres du {last_cutoff_start_str} au {last_cutoff_end_str} - Responsabilité partielle"
        )
