...
def fill_detailed_claims():
    ...
    column_config = configure_column_config(
    ...
    df_resp_claims = df_claims[
        df_claims["claim_resp"] != "Non responsable"
...
def configure_column_config(date_min, date_max, resp_options):
    config = {
        "claim_date": st.column_config.DateColumn(
            ...
            max_value=date_max,
            required=True,
            default=date_max - pd.DateOffset(months=2) - pd.DateOffset(days=1),
        ),
